...
<div class="form-group">
  ...
  <input type="text" name="nama_event" id="nama_event" value="{{ $data->nama_event }}" class="form-control bg-dark border-dark text-white" autocomplete="off">
</div>
<div class="form-group">
...
<div class="form-group">
  ...
  <input type="text" name="lokasi" id="lokasi" class="form-control bg-dark border-dark text-white" placeholder="Contoh: Sumatera Utara Main Stadium" value="{{ @$data->lokasi }}" autocomplete="off">
</div>

<div class="form-group">
  <label for="kordinat">Kordinat</label>
  <input type="text" name="kordinat" id="kordinat" class="form-control bg-dark border-dark text-white" placeholder="{{ @$data->kordinat[0] }}, {{ @$data->kordinat[1] }}" value="{{ @$data->kordinat[0] }}, {{ @$data->kordinat[1] }}" autocomplete="off">
</div>
...
<script>
  $(() => {
    ...
    @for($i = 0; $i < count($data->foto); $i++)
      append_container_image_picker({{ $i }})

      $('[name="foto[{{ $i }}][value]"]').val("{{ $data->foto[$i]->value }}")
      generate_image_handler({{ $i }}, "{{ asset('storage/'.$data->foto[$i]->value) }}")
    @endfor
    ...
    flatpickr('#waktu_event', {
      ...
      defaultDate: moment("{{ $data->waktu_event }}", "DD-MM-YYYY HH:mm:ss").toDate(),
      altInput: true,
  ...
  $(() => {
    ...
    setTimeout(() => {
      // Buat peta
      var map = L.map(`map-0`).setView([{{ @$data->kordinat[0] }}, {{ @$data->kordinat[1] }}], 15);
